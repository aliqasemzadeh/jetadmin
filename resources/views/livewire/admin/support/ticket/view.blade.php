<div>
    ...
    <x-slot name="breadcrumb">
        <ol class="breadcrumb breadcrumb-arrows" aria-label="breadcrumbs">
            ...
            <li class="breadcrumb-item"><a href="{{ route('admin.support.ticket.index') }}">{{ __('jetadmin::bap.tickets') }}</a></li>
    ...
    @if($ticket->status == 'done' || $ticket->status == 'close' || $ticket->status == 'closed')
        <div class="alert alert-important alert-info alert-dismissible" role="alert">
            <div class="d-flex">
                ...
                <div>
                    {{ __('jetadmin::bap.please_create_new_ticket') }}
                </div>
            </div>
        ...
        <div class="col-12">
            <form wire:submit.prevent="replay" class="card">
                <div class="card-header">
                    <h4 class="card-title">{{ __('jetadmin::bap.replay') }}</h4>
                </div>
                <div class="card-body">

                    <div class="mb-3">
                        <label class="form-label" for="description">{{ __('jetadmin::bap.body') }}</label>
                    ...
                    <div class="mb-3">
                        <div class="form-label">{{ __('jetadmin::bap.files') }}</div>
                ...
                <div class="card-footer text-end">
                    <div class="d-flex">
                        <button type="submit" wire:model="next_action" value="index" class="btn btn-secondary">{{ __('jetadmin::bap.submit') }}</button>
                        <button type="submit" name="next_action" wire:model="next_action" value="next" class="btn btn-primary ms-auto">{{ __('jetadmin::bap.next_submit') }}</button>
                    </div>
                </div>
            </form>
        </div>
