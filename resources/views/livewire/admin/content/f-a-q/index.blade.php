<div>
    <x-slot name="title">
        {{ __('jetadmin::bap.faqs') }}
    </x-slot>
    <x-slot name="actions">
        @can('admin_faq_create')
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list">
                    <button onclick="Livewire.emit('showModal', 'admin.content.f-a-q.create')" class="btn btn-primary d-none d-sm-inline-block">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
                        {{ __('jetadmin::bap.create_faq') }}
                    </button>
                    <button onclick="Livewire.emit('showModal', 'admin.content.f-a-q.create')" class="btn btn-primary d-sm-none btn-icon" aria-label="Create new report">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
                    </button>
                </div>
            </div>
    ...
    <x-slot name="breadcrumb">
        <ol class="breadcrumb breadcrumb-arrows" aria-label="breadcrumbs">
            ...
            <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('admin.content.faq.index') }}">{{ __('jetadmin::bap.faqs') }}</a></li>
        </ol>
    </x-slot>

    <div class="card">
        <div class="card-header">
            <h3 class="card-title">{{ __('jetadmin::bap.faqs') }}</h3>
        </div>
        <div class="card-body">
            <div class="d-flex">
        ...
        <div class="table-responsive">
            <table class="table card-table table-vcenter text-nowrap datatable">
                ...
                <tr>
                    ...
                    <th class="w-1" wire:click="sortByColumn('id')">{{ __('jetadmin::bap.number') }}
                    ...
                    <th wire:click="sortByColumn('question')">{{ __('jetadmin::bap.question') }}
                    ...
                    <th wire:click="sortByColumn('answer')">{{ __('jetadmin::bap.answer') }}
                    @if ($sortColumn == 'answer')
                        @if($sortDirection == 'asc')
                            <!-- Download SVG icon from http://tabler-icons.io/i/chevron-up -->
                                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-sm text-dark icon-thick" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><polyline points="6 15 12 9 18 15" /></svg>
                        @else
                ...
                @foreach($faqs as $faq)
                    <tr>
                        <td><input class="form-check-input m-0 align-middle" type="checkbox" aria-label="Select User" value="{{ $faq->id }}" name="selectedItems" wire:model="selectedItems"></td>
                        <td>{{ $faq->id }}</td>
                        <td>{{ $faq->question }}</td>
                        <td>{{ $faq->answer }}</td>
                        <td class="text-end">
                            @can('admin_article_edit')
                                <button onclick="Livewire.emit('showModal', 'admin.content.f-a-q.edit', '{{ json_encode($faq->id) }}')" class="btn btn-primary btn-icon btn-sm">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M9 7h-3a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-3" /><path d="M9 15h3l8.5 -8.5a1.5 1.5 0 0 0 -3 -3l-8.5 8.5v3" /><line x1="16" y1="5" x2="19" y2="8" /></svg>
                                </button>
                            @endcan
                            @can('admin_faq_delete')
                                <button wire:click="delete({{ $faq->id }})" class="btn btn-danger btn-icon btn-sm">
        ...
        <div class="card-footer d-flex justify-content-between">
            <div>
                <div class="btn-group btn-group-sm w-100">
                    <button type="button" wire:click="deleteSelected" class="btn">{{ __('jetadmin::bap.delete') }} ({{ count($selectedItems) }})</button>
                </div>
