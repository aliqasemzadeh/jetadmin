<div>
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list">
                    <button onclick="Livewire.emit('showModal', 'admin.content.carousel.create')" class="btn btn-primary d-none d-sm-inline-block">
                        ...
                        {{ __('jetadmin::bap.create_carousel') }}
                    </button>
                    <button onclick="Livewire.emit('showModal', 'admin.content.carousel.create')" class="btn btn-primary d-sm-none btn-icon" aria-label="{{ __('jetadmin::bap.create_carousel') }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
                    </button>
                </div>
            </div>
    ...
    <x-slot name="breadcrumb">
        <ol class="breadcrumb breadcrumb-arrows" aria-label="breadcrumbs">
            ...
            <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('admin.content.carousel.index') }}">{{ __('jetadmin::bap.carousels') }}</a></li>
        </ol>
    </x-slot>
        ...
        <div class="table-responsive">
            <table class="table card-table table-vcenter text-nowrap datatable">
                ...
                <tr>
                    ...
                    <th wire:click="sortByColumn('title')">{{ __('jetadmin::bap.title') }}
                    @if ($sortColumn == 'title')
                        @if($sortDirection == 'asc')
                            <!-- Download SVG icon from http://tabler-icons.io/i/chevron-up -->
                                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-sm text-dark icon-thick" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><polyline points="6 15 12 9 18 15" /></svg>
                    ...
                    <th wire:click="sortByColumn('user_id')">{{ __('jetadmin::bap.user') }}
                ...
                @foreach($carousels as $carousel)
                    <tr>
                        <td><input class="form-check-input m-0 align-middle" type="checkbox" aria-label="Select User" value="{{ $carousel->id }}" name="selectedItems" wire:model="selectedItems"></td>
                        <td>{{ $carousel->id }}</td>
                        <td>
                            <div class="d-flex py-1 align-items-center">
                                <span class="avatar me-2" style="background-image: url({{ $carousel->getMedia()[0]->getFullUrl() }})"></span>
                                <div class="flex-fill">
                                    <div class="font-weight-medium">{{ $carousel->title }}</div>
                                    <div class="text-muted">{{ $carousel->description }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="text-end">
                            ...
                            @endcan
                            @can('admin_carousel_delete')
                                <button wire:click="delete({{ $carousel->id }})" class="btn btn-danger btn-icon btn-sm">
