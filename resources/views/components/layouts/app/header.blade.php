<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark" dir="{{ __('jetadmin.direction') }}">
    <head>
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            @if(\Illuminate\Support\Facades\Route::is('user.*') || \Illuminate\Support\Facades\Route::is('dashboard'))
                @can('user_access')
                    @include('components.layouts.menus.user.header')
                @endcan
            @endif
            ...
            @can('administrator_access')
                @if(\Illuminate\Support\Facades\Route::is('administrator.*'))
                    @include('components.layouts.menus.administrator.header')
                @endif
            @endcan
            ...
            <flux:dropdown position="top" align="end">
                ...
                <flux:menu>
                    ...
                    <flux:menu.radio.group>
                        <flux:menu.item :href="route('user.settings.profile')" :current="request()->routeIs('user.settings.profile')" icon="cog" wire:navigate>{{ __('Settings') }}</flux:menu.item>
                    </flux:menu.radio.group>
